...
(4,'以下选项不属于C语言的类型的是（）',4,4,'signedshortint','unsignedlongint','unsignedint','longshort'),
(5,'在C语言中，反斜杠符是（）',4,3,'\\n','\\t','\\\\','\\v'),
...
(18,'Git的哪条命令可以将远端库的信息拉取到本地库？',4,3,'gitbranch','gitcheckout','gitfetch','gitmerge'),
(19,'在本地做git操作，如果这时网络中断了，还能提交(commit)代码吗？',2,1,'能','不能','',''),
(20,'如果把项目中文件hello.c的内容破坏了，如何使其还原至原始版本？',4,2,'gitreset--hello.c','gitcheckoutHEAD--hello.c','gitreverthello.c','gitupdatehello.c'),
(21,'在CodeClub上将一个分支合并到另一个分支时需要提交合并请求，请问这个合并请求在CodeClub上的术语是什么？',4,2,'PullRequest','MergeRequest','gitmerge','MergeBranch'),
(22,'在CodeClub上，如果我看到别人的公开项目想要修改代码，但自己又不在项目成员列表里，哪种方法最合适？',3,2,'Clone代码到本地，修改后找项目管理员帮忙上库','先Fork项目到自己名下，然后修改，再提MergeRequest','找项目成员借账号修改代码上库',''),
(23,'Stackoverflow.com是一个什么网站？',4,4,'看名字就知道是讨论堆栈使用的技术网站','栈溢出，钓鱼网站，内含病毒','程序员交友网站','程序员问答网站'),
(24,'被誉为软件设计圣经的书《设计模式》由四人合作完成，他们也因此被成为四人帮（GangOfFour），请问这四人分别是：',4,4,'西施，貂蝉，昭君，玉环','江青、王洪文、张春桥、姚文元','JohnLennon,PaulMcCartney,GeorgeHarrison,RingoStarr','ErichGamma,RichardHelm,RalphJohnson，JohnVlissides'),
(25,'敏捷流派中极限编程(ExtremeProgramming)名称的由来：',4,3,'创立者KentBeck喜欢极限运动','创立者喜欢极端乐队(Extreme)','如果一项实践是好的，那就把它做到极致','创立者极端自恋'),
...
(52,'日志注入的攻击方式与下列哪类攻击最相似：',4,3,'XSS','CSRF','CRLFInjection','DoS'),
...
(69,'某进程收socket消息时，因为该线程就只做收消息这一件事，可以使用不带超时参数的接口selectfds。',2,2,'对','错','',''),
...
(102,'《人月神话》这本书的作者是谁？',3,2,'DennisM.Ritchie','FredBrooks','BrianKernighan',''),
...
(115,'如下哪种工具能够统计软件圈复杂度？',2,1,'SourceMonitor','SourceInsight','',''),
...
(120,'对名为fido的文件用chmod551fido进行了修改，则它的许可权是（）。',4,4,'-rwxr-xr-x','-rwxr--r--','-r--r--r--','-r-xr-x—x'),
